...
const TR = {
  ja: {
    ...
    orderSuccess: '注文を送信しました',
    callFailed: 'スタッフ呼び出しに失敗しました',
    callSuccess: 'スタッフを呼び出しました',
    paymentFailed: '会計リクエストに失敗しました',
    paymentSuccess: '会計リクエストを送りました',
    tableLoadFailed: 'テーブル情報を取得できませんでした',
    closedTitle: '現在営業時間外です',
    closedSub: '営業時間内にまたお越しください',
  ...
  en: {
    ...
    payDoneSub: 'Thank you! See you again 🍜',
    total: 'Total',
    confirm2: 'Confirm',
    ...
    callSuccess: 'Staff has been called',
    paymentFailed: 'Failed to request payment',
    paymentSuccess: 'Payment request sent',
    tableLoadFailed: 'Failed to load table information',
    closedTitle: 'Currently Closed',
    closedSub: 'Please come back during business hours',
  ...
  ko: {
    ...
    orderDoneSub: '주방에 전달되었습니다. 곧 가져다 드리겠습니다.',
    seeReceipt: '계산 보기',
    addMore: '추가 주문',
    callTitle: '직원 호출',
    callSub: '용건을 선택하고 버튼을 눌러주세요',
    callMemo: '메모 (선택사항)',
    callBtn: '🔔 직원 호출',
    callSent: '직원에게 알렸습니다',
    ...
    orderFailed: '주문 전송에 실패했습니다',
    orderSuccess: '주문이 접수되었습니다',
    callFailed: '직원 호출에 실패했습니다',
    callSuccess: '직원을 호출했습니다',
    paymentFailed: '결제 요청에 실패했습니다',
    paymentSuccess: '결제 요청을 보냈습니다',
    tableLoadFailed: '테이블 정보를 불러오지 못했습니다',
    closedTitle: '현재 영업 시간 외입니다',
    closedSub: '영업 시간에 다시 방문해 주세요',
  ...
  zh: {
    ...
    callFailed: '呼叫服务员失败',
    callSuccess: '已呼叫服务员',
    paymentFailed: '发送结账请求失败',
    paymentSuccess: '已发送结账请求',
    tableLoadFailed: '无法获取桌台信息',
    closedTitle: '当前非营业时间',
    closedSub: '请在营业时间内再次光临',
...
function OrderForm() {
    ...
    try {
      ...
      if (tableId && sessionId) {
        try {
          ...
          allOrdersSnapshot.docs.forEach((orderDoc) => {
      ...
      setOrderStep('done');
      setCart({});
      setSuccessMessage(t.orderSuccess);
    } catch (error) {
      console.error(error);
  ...
  const handleCall = async () => {
    ...
    if (!selectedCall) {
      setErrorMessage(t.selectCallReason);
      return;
    }
    ...
    try {
      ...
      setCallMemo('');
      setSuccessMessage(t.callSuccess);
  ...
  const handlePayment = async () => {
    ...
    if (orderedItems.length === 0) {
      setErrorMessage(t.noOrderToPay);
      return;
    }
    ...
    >
      {!isOpen && (
        ...
        >
          ...
          <div style={{ fontSize: '14px', color: '#888891', textAlign: 'center' }}>{t.closedSub}</div>
        </div>
      )}
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          {totalQty > 0 && activeTab === 'order' && (
            ...
            >
              <span style={{ color: C.amber, fontSize: '12px', fontWeight: 700 }}>
                🛒 {totalQty}点 ¥{total.toLocaleString()}
              </span>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            {(['ja', 'en', 'ko', 'zh'] as Lang[]).map((l) => (
              <button
                ...
                onClick={() => setLang(l)}
                style={{
                  background: lang === l ? C.amberD : C.faint,
                  border: `1px solid ${lang === l ? C.amberM : C.bdr}`,
                  borderRadius: '6px',
                  padding: '4px 6px',
                  color: lang === l ? C.amber : C.muted,
                  fontSize: '10px',
                  fontWeight: 700,
                  cursor: 'pointer',
                  fontFamily: 'inherit',
      ...
      >
        ...
        <button style={TAB_STYLE(activeTab === 'call')} onClick={() => setActiveTab('call')}>
          {t.callTitle}
        </button>
        <button style={TAB_STYLE(activeTab === 'receipt')} onClick={() => setActiveTab('receipt')}>
          {t.receipt}
        </button>
      </div>
          ...
          {errorMessage && (
            <div
              style={{
                ...
                fontSize: '13px',
                whiteSpace: 'pre-line',
                marginBottom: successMessage ? '8px' : 0,
              }}
            >
              {errorMessage}
            </div>
          ...
          {orderStep === 'done' ? (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                <div style={{ fontSize: '56px', marginBottom: '16px' }}>✅</div>
                <div style={{ fontSize: '22px', fontWeight: 800, color: C.txt, marginBottom: '8px' }}>
                  {t.orderDone}
                </div>
                ...
                >
                  {cartItems.map((item) => (
                    ...
                    >
                      ...
                      <p style={{ color: C.amber, fontWeight: 700, fontSize: '14px', margin: 0 }}>
                  ...
                  >
                    ...
                    <p style={{ color: C.amber, fontWeight: 700, fontSize: '22px', margin: 0 }}>
                ...
                {categories.map((cat) => (
                  <button
                    ...
                    style={{
                      ...
                      border: 'none',
                      borderBottom:
                        activeCategory === cat ? `2px solid ${C.amber}` : '2px solid transparent',
                      cursor: 'pointer',
                      whiteSpace: 'nowrap',
                      fontFamily: "'Noto Sans JP', sans-serif",
                    }}
                ...
                {menuItems
                  ...
                  .map((item) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <p style={{ color: C.txt, fontWeight: 700, fontSize: '14px', margin: '0 0 2px' }}>
                          {item.name}
                        </p>
                        <p style={{ color: C.amber, fontWeight: 700, fontSize: '13px', margin: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span
                          style={{
                            fontSize: '14px',
                            fontWeight: 700,
                            color: cart[item.id] > 0 ? C.amber : C.muted,
                            minWidth: '16px',
                            textAlign: 'center',
                          }}
                        >
      ...
      {activeTab === 'call' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
          ...
          {callOptions.map((opt) => (
            <button
              key={opt.id}
              onClick={() => setSelectedCall(selectedCall === opt.id ? null : opt.id)}
              style={{
                width: '100%',
                background: selectedCall === opt.id ? C.amberD : C.surf,
                border: `1px solid ${selectedCall === opt.id ? C.amberM : C.bdr}`,
                borderRadius: '12px',
                padding: '14px 16px',
                marginBottom: '8px',
                color: selectedCall === opt.id ? C.amber : C.txt,
                fontSize: '14px',
                fontWeight: 700,
                cursor: 'pointer',
                textAlign: 'left',
          ...
          {callSent ? (
            ...
            <button
              onClick={handleCall}
              disabled={!selectedCall || loading}
              style={{
                width: '100%',
                background: selectedCall ? C.amber : C.faint,
                border: 'none',
                color: selectedCall ? C.bg : C.muted,
                fontSize: '15px',
                fontWeight: 800,
                padding: '14px',
                borderRadius: '12px',
                cursor: selectedCall ? 'pointer' : 'default',
                fontFamily: "'Noto Sans JP', sans-serif",
              }}
            >
              {t.callBtn}
      ...
      {activeTab === 'receipt' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
          {payDone ? (
            <div style={{ textAlign: 'center', padding: '50px 20px' }}>
              ...
              >
                ...
                >
                  ...
                  <span style={{ color: C.amber, fontSize: '22px', fontWeight: 800 }}>
            ...
            <>
              <div style={{ fontSize: '15px', fontWeight: 700, color: C.txt, marginBottom: '14px' }}>
                {t.orderContents}
              </div>
              ...
              <div style={{ fontSize: '13px', fontWeight: 700, color: C.txt, marginBottom: '10px' }}>
                {t.payMethod}
              </div>
                ...
                {(['card', 'cash'] as const).map((method) => (
                  <button
                    ...
                    style={{
                      padding: '14px',
                      borderRadius: '12px',
                      background: payMethod === method ? C.amberD : C.surf,
                      border: `1px solid ${payMethod === method ? C.amberM : C.bdr}`,
                      color: payMethod === method ? C.amber : C.muted,
                      fontSize: '14px',
                      fontWeight: 700,
                      cursor: 'pointer',
                      fontFamily: "'Noto Sans JP', sans-serif",
              ...
              <button
                ...
                disabled={orderedItems.length === 0 || loading}
                style={{
                  width: '100%',
                  background: orderedItems.length > 0 ? C.amber : C.faint,
                  border: 'none',
                  color: orderedItems.length > 0 ? C.bg : C.muted,
                  fontSize: '15px',
                  fontWeight: 800,
                  padding: '14px',
                  borderRadius: '12px',
                  cursor: orderedItems.length > 0 ? 'pointer' : 'default',
                  fontFamily: "'Noto Sans JP', sans-serif",
                }}
              >
                {t.payBtn}
